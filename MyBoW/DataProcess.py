...
def plotCM(classes, matrix, savename):
    ...
    matrix /= linesum   # number to probe
...
def getDatasetFromFile(path):
    ...
    label = 0 # begin with zero
    feature_set = np.float32([]).reshape(0, 128)
    for dir in dirs:
        files = os.listdir(path + "/" + dir)
        #files.remove("Thumbs.db")

        for i in range(150):    # train data
        # ----------------------write your code bellow----------------------
            ...
            feature_set = np.append(feature_set,img_info.descriptors,axis = 0) # add sift desc to feature_set
        # ----------------------write your code above----------------------

        for i in range(150, len(files)): # test data
        # ----------------------write your code bellow----------------------
            img = cv2.imread(path + "/" + dir + "/" + files[i])
            img_info = ImageInfo.ImageInfo(img,label)
            img_info.getImgFeature()
